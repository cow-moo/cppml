...
public:
    ...
    T read_at(size_t offset) const override {
        if (offset >= size_) throw std::out_of_range("CpuMultiThreadBuffer::read_at");
        return data_[offset];
    }
    ...
    void reduce(const Shape& rShape, const Strides& rStrides, size_t rOffset,
        ...
        if (intermediateDim >= 2) {
            ...
            for (size_t i = 0; i < rShape.numel(); i++) {
                *rIt = intermediate[i * intermediateDim];
                for (size_t j = 1; j < intermediateDim; j++) {
    ...
    // TODO: implement with multithreading
